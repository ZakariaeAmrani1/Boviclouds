...
interface AddSujetModalProps {
  ...
  isLoading: boolean;
  currentTab: TraitementTab;
}

const AddSujetModal: React.FC<AddSujetModalProps> = ({
  isOpen,
  onClose,
  onSubmit,
  isLoading,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (currentTab === "identification") {
      onSubmit(baseData as CreateSujetRequest);
    } else if (currentTab === "insemination") {
      onSubmit(baseData as CreateInseminationRequest);
    } else if (currentTab === "lactation") {
      onSubmit({
        ...baseData,
        productionJournaliere: parseFloat(baseData.productionJournaliere) || 0,
      } as CreateLactationRequest);
    }
  };
  ...
  const getModalTitle = () => {
    switch (currentTab) {
      case "identification":
        return "Ajouter une Identification";
      case "insemination":
        return "Ajouter une Insémination";
      case "lactation":
        return "Ajouter une Lactation";
      default:
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid gap-6">
            {currentTab === "identification" && renderIdentificationForm()}
            {currentTab === "insemination" && renderInseminationForm()}
            {currentTab === "lactation" && renderLactationForm()}
          </div>
