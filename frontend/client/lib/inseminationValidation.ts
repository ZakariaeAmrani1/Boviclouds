  ...
  .superRefine((data, ctx) => {
    ...
    if (data.inseminateur_id === data.responsable_local_id) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "L'inséminateur et le responsable local doivent être différents",
...
export const UpdateInseminationValidationSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.date_dissemination) {
      ...
      if (disseminationDate < fiveYearsAgo) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message:
            "La date de dissémination ne peut pas être antérieure à 5 ans",
          path: ["date_dissemination"],
        });
      }
    }
    ...
    if (
      data.inseminateur_id &&
      data.responsable_local_id &&
