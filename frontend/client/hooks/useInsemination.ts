...
export const useInseminationList = (
  ...
  const [data, setData] = useState<PaginatedResponse<InseminationRecord> | null>(null);
...
export const useInsemination = () => {
  ...
  const createRecord = useCallback(async (input: CreateInseminationInput): Promise<InseminationRecord | null> => {
    setLoading(true);
    setError(null);
    try {
  ...
  const updateRecord = useCallback(async (id: string, input: UpdateInseminationInput): Promise<InseminationRecord | null> => {
    setLoading(true);
    setError(null);
    try {
  ...
  const getRecord = useCallback(async (id: string): Promise<InseminationRecord | null> => {
    setLoading(true);
    setError(null);
    try {
...
export const useInseminationSearch = () => {
  ...
  const searchByNNI = useCallback(async (nni: string): Promise<InseminationRecord[]> => {
    if (!nni.trim()) return [];
  ...
  const searchBySemenceId = useCallback(async (semenceId: string): Promise<InseminationRecord[]> => {
  ...
  const checkDuplicateInsemination = useCallback(async (
    nni: string, 
    date: string, 
    excludeId?: string
  ): Promise<boolean> => {
    ...
    setError(null);
    try {
      const result = await InseminationService.checkDuplicateInsemination(nni, date, excludeId);
      return result;
    } catch (err) {
